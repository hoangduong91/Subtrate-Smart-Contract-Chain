...
parameter_types! { 
	pub  SystemChainId: u64 = sp_io::misc::chain_id();
}

impl evm::Trait for Runtime { 
	type FeeCalculator = FixedGasPrice;
	type CallOrigin = evm::EnsureAddressRoot<AccountId>;
	type WithdrawOrigin = evm::EnsureAddressNever<AccountId>;
	type AddressMapping = evm::HashedAddressMapping<sp_runtime::traits::BlakeTwo256>;
	type Currency = Balances;
	type Event = Event;
	type Precompiles = ();
...
construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		Sudo: pallet_sudo::{Module, Call, Config<T>, Storage, Event<T>},		Contracts: contracts::{Module, Call, Config, Storage, Event<T> },
		Evm: evm::{Module, Storage, Event<T>, Config },
